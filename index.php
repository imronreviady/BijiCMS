...
if (file_exists('install.php')) {
	...
	*/
	if (strpos($_SERVER['REQUEST_URI'], DIR_ADM) !== false) {
		...
		/**
		 * Mendeklarasi class Core
		 *
		 * Declaration Core class
		 *
		*/
		$core = new Core();
		...
		*/
		if (isset($_POST['lang'])) {
			$check_lang = $core->db->from('language')
				->where('code', $core->string->valid($_POST['lang'], 'xss'))
				->limit(1)
				->fetch();
			if ($check_lang) {
				setcookie('lang_front', $core->string->valid($_POST['lang'], 'xss'), 1719241200, '/');
				define('WEB_LANG_ID', $check_lang['id_language']);
				define('WEB_LANG', $check_lang['code']);
			} else {
				$current_lang = $core->db->from('language')
					->where('id_language', '1')
					->limit(1)
					->fetch();
			...
			$check_lang = $core->db->from('language')
				->where('code', $core->string->valid($_GET['lang'], 'xss'))
				->limit(1)
				->fetch();
			if ($check_lang) {
				setcookie('lang_front', $core->string->valid($_GET['lang'], 'xss'), 1719241200, '/');
				define('WEB_LANG_ID', $check_lang['id_language']);
				define('WEB_LANG', $check_lang['code']);
			} else {
				$current_lang = $core->db->from('language')
		...
		} elseif (isset($_COOKIE['lang_front'])) {
			$current_lang = $core->db->from('language')
				->where('code', $_COOKIE['lang_front'])
				->limit(1)
				->fetch();
			define('WEB_LANG_ID', $current_lang['id_language']);
			define('WEB_LANG', $current_lang['code']);
		...
		*/
		...
		if ($core->request->check_internet_connection()) {
		...
		$statistics = $core->db->from('traffic')
			->where('ip', $ip_stat)
			...
			->count();
		if ($statistics > 0) {
			$current_statistic = $core->db->from('traffic')
				->where('ip', $ip_stat)
				->where('country', $country_stat)
				->where('city', $city_stat)
				->where('date', $date_stat)
			...
			$query_stat = $core->db->update('traffic')
				->set($data_stat)
				->where('ip', $ip_stat)
				->where('country', $country_stat)
				->where('city', $city_stat)
			...
			$query_stat = $core->db->insertInto('traffic')->values($data_stat);
			$query_stat->execute();
		}

		/**
		 * Mendeklarasi class Router
		 *
		 * Declaration Router class
		 *
		*/
		$router = new Router();
		...
		*/
		if (strpos($_SERVER['REQUEST_URI'], 'member') !== false) {
			...
			$active_template = "/".$core->theme['folder'];
		}
		$templates = new Template\Engine(DIR_CON.'/themes'.$active_template);
		$templates->loadExtension(new Template\Extension\Core());
		$templates->loadExtension(new Template\Extension\Asset(DIR_CON.'/themes'.$active_template));

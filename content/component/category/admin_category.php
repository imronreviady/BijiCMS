	...
	*/
	public function index()
	{
		...
		<div class="block-content">
			...
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->formStart(array('method' => 'post', 'action' => 'route.php?mod=category&act=multidelete', 'autocomplete' => 'off'));?>
						...
						?>
						<?=$this->html->createTable(array('id' => 'table-category', 'class' => 'table table-striped table-bordered'), $columns, $tfoot = true);?>
					<?=$this->html->formEnd();?>
				</div>
			</div>
		</div>
		<?=$this->html->dialogDelete('category');?>
		<?php
	}
	...
	public function datatable()
	{
		...
		$columns = array(
			...
			array('db' => 'cd.title', 'dt' => '3', 'field' => 'title',
				'formatter' => function($d, $row, $i){
					return "".$d."<br /><i><a href='".WEB_URL."category/".$this->string->seo_title($d)."' target='_blank'>".WEB_URL."category/".$this->string->seo_title($d)."</a></i>";
				}
			),
	...
	*/
	public function addnew()
	{
		...
		if (!empty($_POST)) {
			...
			$category = array(
				'id_parent' => $this->string->valid($_POST['id_parent'], 'sql'),
				'seotitle' => $this->string->seo_title($this->string->valid($_POST['category'][1]['title'], 'xss')),
			...
			$this->flash->success($GLOBALS['_']['category_message_1'], 'admin.php?mod=category');
		}
		?>
		<div class="block-content">
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->headTitle($GLOBALS['_']['category_addnew']);?>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->formStart(array('method' => 'post', 'action' => 'route.php?mod=category&act=addnew', 'autocomplete' => 'off'));?>
						<div class="row">
							<div class="col-md-6">
								<?php
							...
							<div class="col-md-6">
								<?=$this->html->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['category_picture'], 'name' => 'picture', 'id' => 'picture'), $inputgroup = true, $inputgroupopt = array('href' => '../'.DIR_INC.'/js/filemanager/dialog.php?type=1&field_id=picture', 'id' => 'browse-file', 'class' => 'btn-success', 'options' => '', 'title' => $GLOBALS['_']['action_7'].' '.$GLOBALS['_']['category_picture']));?>
							</div>
						</div>
						<div class="row">
							<div class="col-md-12">
								...
								<div class="tab-content">
									...
									<div class="tab-pane <?php echo ($noctab == '1' ? 'active' : ''); ?>" id="tab-content-<?=$lang['id_language'];?>">
										<?=$this->html->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['category_title_2'], 'name' => 'category['.$lang['id_language'].'][title]', 'id' => 'title-'.$lang['id_language'], 'mandatory' => true, 'options' => 'required'));?>
									</div>
									<?php $noctab++;} ?>
								</div>
	...
	*/
	public function edit()
	{
		...
		if (!empty($_POST)) {
			...
			$query_category = $this->db->update('category')
				->set($category)
				->where('id_category', $this->string->valid($_POST['id'], 'sql'));
			$query_category->execute();
			foreach ($_POST['category'] as $id_language => $value) {
				$othlang_category = $this->db->from('category_description')
					->where('id_category', $this->string->valid($_POST['id'], 'sql'))
				...
				if ($othlang_category > 0) {
					...
					$query_category_description = $this->db->update('category_description')
						->set($category_description)
						->where('id_category_description', $this->string->valid($value['id'], 'sql'));
				} else {
					$category_description = array(
						'id_category' => $this->string->valid($_POST['id'], 'sql'),
						'id_language' => $id_language,
						'title' => $this->string->valid($value['title'], 'xss')
					);
					$query_category_description = $this->db->insertInto('category_description')->values($category_description);
			...
			$this->flash->success($GLOBALS['_']['category_message_2'], 'admin.php?mod=category');
		}
		$id = $this->string->valid($_GET['id'], 'sql');
		$current_category = $this->db->from('category')
			->select('category_description.title')
			->leftJoin('category_description ON category_description.id_category = category.id_category')
		...
		<div class="block-content">
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->headTitle($GLOBALS['_']['category_edit']);?>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->formStart(array('method' => 'post', 'action' => 'route.php?mod=category&act=edit&id='.$current_category['id_category'], 'autocomplete' => 'off'));?>
						<?=$this->html->inputHidden(array('name' => 'id', 'value' => $current_category['id_category']));?>
						<div class="row">
							<div class="col-md-6">
								<?php
									$selcats = $this->db->from('category')
										->select('category_description.title')
										->leftJoin('category_description ON category_description.id_category = category.id_category')

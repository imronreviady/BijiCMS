...
class Pages extends Core
{
	...
	*/
	public function index()
	{
		...
		<div class="block-content">
			...
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->formStart(array('method' => 'post', 'action' => 'route.php?mod=pages&act=multidelete', 'autocomplete' => 'off'));?>
						...
						?>
						<?=$this->html->createTable(array('id' => 'table-pages', 'class' => 'table table-striped table-bordered'), $columns, $tfoot = true);?>
					<?=$this->html->formEnd();?>
				</div>
			</div>
		</div>
		<?=$this->html->dialogDelete('pages');?>
		<?php
	}
	...
	public function addnew()
	{
		...
		if (!empty($_POST)) {
			if ($_POST['seotitle'] != "") {
				...
				$seotitle = $this->string->seo_title($this->string->valid($_POST['pages'][1]['title'], 'xss'));
			}
			...
			$query_pages = $this->db->insertInto('pages')->values($pages);
			$query_pages->execute();
			$last_pages = $this->db->from('pages')
			...
			foreach ($_POST['pages'] as $id_language => $value) {
				...
				$query_pages_description = $this->db->insertInto('pages_description')->values($pages_description);
				$query_pages_description->execute();
			}
			$this->flash->success($GLOBALS['_']['pages_message_1'], 'admin.php?mod=pages');
		}
		?>
		<div class="block-content">
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->headTitle($GLOBALS['_']['pages_addnew']);?>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->formStart(array('method' => 'post', 'action' => 'route.php?mod=pages&act=addnew', 'autocomplete' => 'off'));?>
						<div class="row">
							<div class="col-md-12">
								<?php
									$notab = 1;
								...
								<div class="tab-content">
									...
									<div class="tab-pane <?php echo ($noctab == '1' ? 'active' : ''); ?>" id="tab-content-<?=$lang['id_language'];?>">
										<?=$this->html->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['pages_title_2'], 'name' => 'pages['.$lang['id_language'].'][title]', 'id' => 'title-'.$lang['id_language'], 'mandatory' => true, 'options' => 'required'));?>
										<div class="form-group">
											...
											<textarea class="form-control" id="wysiwyg-<?=$lang['id_language'];?>" name="pages[<?=$lang['id_language'];?>][content]" style="height:500px;"></textarea>
										</div>
									</div>
									<?php $noctab++;} ?>
								</div>
						...
						<div class="row">
							<div class="col-md-12">
								<?=$this->html->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['pages_seotitle'], 'name' => 'seotitle', 'id' => 'seotitle', 'mandatory' => true, 'options' => 'required', 'help' => 'Permalink : '.WEB_URL.'pages/<span id="permalink"></span>'));?>
							</div>
						</div>
						<div class="row">
							<div class="col-md-6">
								<?=$this->html->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['pages_picture'], 'name' => 'picture', 'id' => 'picture'), $inputgroup = true, $inputgroupopt = array('href' => '../'.DIR_INC.'/js/filemanager/dialog.php?type=1&field_id=picture', 'id' => 'browse-file', 'class' => 'btn-success', 'options' => '', 'title' => $GLOBALS['_']['action_7'].' '.$GLOBALS['_']['pages_picture']));?>
							</div>
						</div>
						<div class="row">
							<div class="col-md-12">
	...
	*/
	public function edit()
	{
		...
		if (!empty($_POST)) {
			...
			$query_pages = $this->db->update('pages')
				->set($pages)
				->where('id_pages', $this->string->valid($_POST['id'], 'sql'));
			$query_pages->execute();
			foreach ($_POST['pages'] as $id_language => $value) {
				$othlang_pages = $this->db->from('pages_description')
					->where('id_pages', $this->string->valid($_POST['id'], 'sql'))
				...
				if ($othlang_pages > 0) {
					...
					$query_pages_description = $this->db->update('pages_description')
						->set($pages_description)
						->where('id_pages_description', $this->string->valid($value['id'], 'sql'));
				} else {
					$pages_description = array(
						'id_pages' => $this->string->valid($_POST['id'], 'sql'),
						'id_language' => $id_language,
						'title' => $this->string->valid($value['title'], 'xss'),
						'content' => stripslashes(htmlspecialchars($value['content'],ENT_QUOTES))
					);
		...
		<div class="block-content">
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->headTitle($GLOBALS['_']['pages_edit']);?>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<?=$this->html->formStart(array('method' => 'post', 'action' => 'route.php?mod=pages&act=edit&id='.$current_pages['id_pages'], 'autocomplete' => 'off'));?>
						<?=$this->html->inputHidden(array('name' => 'id', 'value' => $current_pages['id_pages']));?>
						<div class="row">
							<div class="col-md-12">
								<?php
									$notab = 1;
								...
								<div class="tab-content">
									...
									<div class="tab-pane <?php echo ($noctab == '1' ? 'active' : ''); ?>" id="tab-content-<?=$lang['id_language'];?>">
										<?php
										$paglang = $this->db->from('pages_description')
										...
										?>
										<?=$this->html->inputHidden(array('name' => 'pages['.$lang['id_language'].'][id]', 'value' => $paglang['id_pages_description']));?>
										<?=$this->html->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['pages_title_2'], 'name' => 'pages['.$lang['id_language'].'][title]', 'id' => 'title-'.$lang['id_language'], 'value' => $paglang['title'], 'mandatory' => true, 'options' => 'required'));?>
										<div class="form-group">
											<label><?=$GLOBALS['_']['pages_content'];?> <span class="text-danger">*</span></label>
											<div class="row" style="margin-top:-30px;">
												<div class="col-md-12">
											...
											<textarea class="form-control" id="wysiwyg-<?=$lang['id_language'];?>" name="pages[<?=$lang['id_language'];?>][content]" style="height:500px;"><?=$content_after;?></textarea>
										</div>
									</div>
									<?php $noctab++;} ?>
								</div>
						...
						<div class="row">
							<div class="col-md-12">
								<?=$this->html->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['pages_seotitle'], 'name' => 'seotitle', 'id' => 'seotitle', 'value' => $current_pages['seotitle'], 'mandatory' => true, 'options' => 'required', 'help' => 'Permalink : '.WEB_URL.'pages/<span id="permalink">'.$current_pages['seotitle'].'</span>'));?>
							</div>
						</div>
						<div class="row">
							<div class="col-md-12">
						...
						<div class="row">
							<div class="col-md-6">
								<?=$this->html->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['pages_picture'], 'name' => 'picture', 'id' => 'picture', 'value' => $current_pages['picture']), $inputgroup = true, $inputgroupopt = array('href' => '../'.DIR_INC.'/js/filemanager/dialog.php?type=1&field_id=picture', 'id' => 'browse-file', 'class' => 'btn-success', 'options' => '', 'title' => $GLOBALS['_']['action_7'].' '.$GLOBALS['_']['pages_picture']));?>
							</div>
						</div>
						<div class="row">
							<div class="col-md-12">
								<?php
									if ($current_pages['active'] == 'N') {
										...
										echo $this->html->inputRadio(array('label' => $GLOBALS['_']['pages_active'], 'mandatory' => true), $radioitem, $inline = true);
									} else {
										$radioitem = array(
	...
	*/
	public function delete()
	{
		...
		if (!empty($_POST)) {
			$query_desc = $this->db->deleteFrom('pages_description')->where('id_pages', $this->string->valid($_POST['id'], 'sql'));
			$query_desc->execute();
			$query_pag = $this->db->deleteFrom('pages')->where('id_pages', $this->string->valid($_POST['id'], 'sql'));
			$query_pag->execute();
			$this->flash->success($GLOBALS['_']['pages_message_3'], 'admin.php?mod=pages');
		}
	}
		...
		if (!empty($_POST)) {
			...
			if ($totaldata != "0") {
				...
				foreach($items as $item){
					$query_desc = $this->db->deleteFrom('pages_description')->where('id_pages', $this->string->valid($item['deldata'], 'sql'));
					$query_desc->execute();
					$query_pag = $this->db->deleteFrom('pages')->where('id_pages', $this->string->valid($item['deldata'], 'sql'));
					$query_pag->execute();
				}
				$this->flash->success($GLOBALS['_']['pages_message_3'], 'admin.php?mod=pages');
			} else {
				$this->flash->error($GLOBALS['_']['pages_message_6'], 'admin.php?mod=pages');
			}
		}
	}

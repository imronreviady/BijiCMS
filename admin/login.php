...
class Login extends Core
{
	...
	*/
	...
	{
		$_POST = $this->val->sanitize($_POST);
		$this->val->validation_rules(array(
			'username' => 'required|max_len,50|min_len,3',
			'password' => 'required|max_len,50|min_len,6'
		));
		$this->val->filter_rules(array(
			'username' => 'trim|sanitize_string',
			'password' => 'trim'
		));
		$validated_data = $this->val->run($_POST);
		if($validated_data === false) {
			header('location:index.php?mod=login&act=index&errormsg=1');
		} else {
			$count_user = $this->db->from('users')
				->where('username', $_POST['username'])
		...
		if($validated_data === false) {
			...
			if ($count_user > 0) {
				...
				$query = $this->db->update('users')
					...
					->where('email', $_POST['email']);
				$query->execute();
				$website_name = $this->setting[0]['value'];
				$website_url = $this->setting[1]['value']."/".DIR_ADM;
				...
				if ($this->setting[23]['value'] != 'SMTP') {
					...
					$this->mail->addAddress($user['email'], $nama_lengkap);
	...
	*/
	public function recovery()
	{
		if (!empty($_GET['forgetuser']) AND !empty($_GET['forgetkey'])){
			$forgetuser = $this->string->valid($_GET['forgetuser'], 'xss');
			$forgetkey = $this->string->valid($_GET['forgetkey'], 'xss');
			$count_user = $this->db->from('users')
